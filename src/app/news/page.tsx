...
export default function NewsPage() {
  ...
  const [filterAudiences, setFilterAudiences] = useState<Set<AudienceLabel>>(
    new Set(),
  );
  ...
  const [selectedItem, setSelectedItem] = useState<NewsDisplayItem | null>(
    null,
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (filterSources.size > 0)
      items = items.filter((n) => filterSources.has(n.source));
    if (filterAudiences.size > 0)
      items = items.filter((n) =>
        n.recommendedFor.some((a) => filterAudiences.has(a)),
      );
    if (searchQuery.trim()) {
  ...
  const currentSortLabel =
    SORT_OPTIONS.find((o) => o.value === sortBy)?.label ?? "최신순";
  ...
  function toggleAudience(aud: AudienceLabel) {
    setFilterAudiences((prev) => {
      const n = new Set(prev);
      n.has(aud) ? n.delete(aud) : n.add(aud);
      return n;
    });
  }
    ...
    <main className="min-h-screen bg-[#f5f6f8]">
      ...
      <section className="bg-[#1e2235] px-6 md:px-16 lg:px-[120px] py-6 md:py-8">
        <div className="max-w-[1200px] mx-auto">
          <p className="text-[#6bb8d4] text-[12px] font-medium tracking-wider mb-2">
            News Feed
          </p>
      ...
      <div
        ...
        onClick={() =>
          allNewsTitleRef.current?.scrollIntoView({
            behavior: "smooth",
            block: "start",
          })
        }
      >
        <div className="max-w-[1200px] mx-auto px-6 md:px-16 lg:px-[120px] py-3 flex items-center gap-2 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[140px] max-w-xs">
            ...
            />
            <svg
              className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#a8b8d0]"
              fill="none"
              stroke="currentColor"
              strokeWidth={2.5}
              viewBox="0 0 24 24"
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#a8b8d0] hover:text-[#3d4f6e] text-[14px] leading-none"
              >
                ✕
              </button>
            )}
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {activeFilterCount > 0 && (
                    <button
                      ...
                      className="text-[11px] text-[#6bb8d4] hover:text-[#1e2235] transition-colors"
                    >
                      초기화
                    </button>
                  )}
                </div>
                <p className="text-[11px] font-bold text-[#8a9bbd] uppercase tracking-wide mb-2">
                  소스
                </p>
                  ...
                  {allSources.map((src) => (
                    ...
                    >
                      <input
                        ...
                        className="w-3.5 h-3.5 accent-[#6bb8d4]"
                      />
                      <span className="text-[13px] text-[#3d4f6e] group-hover:text-[#1e2235] transition-colors">
                        {src}
                      </span>
                    </label>
                  ))}
                ...
                <div className="flex flex-col gap-1.5">
                  {AUDIENCES.map((aud) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={filterAudiences.has(aud)}
          ...
          {/* 결과 수 */}
          {!loading && (
            <span className="text-[12px] text-[#a8b8d0] shrink-0 hidden md:block">
              {filtered.length}건
            </span>
          )}
          ...
          >
            <button
              ...
              className={`px-3 py-2 text-[14px] transition-colors ${viewMode === "grid" ? "bg-[#1e2235] text-white" : "bg-white text-[#3d4f6e] hover:bg-[#f5f6f8]"}`}
              title="카드 보기"
            >
              ⊞
            </button>
            <button
              ...
              className={`px-3 py-2 text-[14px] transition-colors ${viewMode === "list" ? "bg-[#1e2235] text-white" : "bg-white text-[#3d4f6e] hover:bg-[#f5f6f8]"}`}
              title="리스트 보기"
            >
              ☰
            </button>
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-16 lg:px-[120px] py-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-[16px] text-[#1e2235]">
            최근 심각도 TOP 6 뉴스
          </h2>
          <button
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M4 10a8 8 0 0 1 14.9-3M20 14a8 8 0 0 1-14.9 3"
                strokeLinecap="round"
              />
            </svg>
            새로고침
        ...
        {error ? (
          <div className="text-center py-12">
            <p className="text-[#ef4444] text-[14px] mb-1">
              데이터를 불러오지 못했습니다.
            </p>
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-16 lg:px-[120px] py-6">
        <h2
          ref={allNewsTitleRef}
          className="font-bold text-[16px] text-[#1e2235] mb-4 scroll-mt-[150px]"
        >
          전체 뉴스
        </h2>
        {loading ? (
